...
import authorsPageInitialPaths2bcfcResource from '../../resources/authors-page-initial-paths-2bcfc'
import authorsPageInitialProps017c3Resource from '../../resources/authors-page-initial-props-017c3'
  ...
  return (
    <>
      <div className="authors11-container">
        <Head>
          <title>Authors1 - Future Metrics Designer</title>
          <meta
            property="og:title"
            content="Authors1 - Future Metrics Designer"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPaths2bcfcResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialProps017c3Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
