...
import postsPageInitialPaths42be2Resource from '../../resources/posts-page-initial-paths-42be2'
import postsPageInitialProps12cdaResource from '../../resources/posts-page-initial-props-12cda'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Future Metrics Designer</title>
          <meta
            property="og:title"
            content="Posts1 - Future Metrics Designer"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths42be2Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps12cdaResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
